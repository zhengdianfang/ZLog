...
### Step 3 — Create or Update Trello Card

**New card:** Invoke the `trello-cli` skill with `operation: create` to create a card in the TODO column.

**Existing card with new requirements:** Update the card description via the Trello API (PUT /cards/{id}), then move the card to the **DOING** column immediately — it is going back into active development.
